import { div, Property, ReactiveElement, ReactiveElementProps, Register, VDOMElement, WithBinding } from '@io-gui/core'
import { ioDrawerHandle, IoDrawerHandle } from '@io-gui/layout'
...
function parseDrawerSizePx(size: string, containerHeight: number): number {
  const pxMatch = size.match(/^([\d.]+)px$/)
  if (pxMatch) return parseFloat(pxMatch[1])
  const pctMatch = size.match(/^([\d.]+)%$/)
  if (pctMatch) return containerHeight * parseFloat(pctMatch[1]) / 100
  return 330
}

@Register
export class BottomDrawerSplit extends ReactiveElement {
  ...
  @Property({ type: Boolean, value: false, reflect: true })
  declare intro: boolean

  @Property({ type: String, value: '' })
  declare revealSelector: string

  #revealObserver?: ResizeObserver
  #introPlayed = false
  #introTimeout?: number
  ...
  expandedChanged() {
    const handle = this.$['handle'] as IoDrawerHandle | undefined
    if (handle) {
...
export const bottomDrawerSplit = function(props: BottomDrawerSplitProps) {
