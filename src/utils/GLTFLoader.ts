/* eslint-disable @typescript-eslint/no-this-alias, no-useless-escape */
...
import type { LoadingManager, Camera } from 'three'
...
/** Aggregate download progress callback used by this vendored loader. */
export type TrackProgress = (resourceUrl: string) => (event: ProgressEvent) => void


/**
 * Vendored from three@0.185.0 `examples/jsm/loaders/GLTFLoader.js`.
 * Extended to aggregate download progress across the root asset and external buffers.
 *
...
export class GLTFLoader extends Loader<GLTF> {
	...
	pluginCallbacks: Array<(parser: GLTFParser) => any> = []
	 ...
	 */
	load( url: string, onLoad: ( gltf: GLTF ) => void, onProgress?: ( event: ProgressEvent ) => void, onError?: ( event: ErrorEvent | Error | unknown ) => void ): void {
		...
		// Aggregate byte progress across the root glTF/GLB and any external buffers.
		...
		// Single-file GLBs need root progress; split .gltf+.bin gets aggregate progress after parse.
		const trackRootProgress = /\.glb(\?|$)/i.test( url ) ? trackProgress( url ) : undefined
		...
		loader.load( url, function ( data: any) {
			...
			try {
				...
				// briefly report 100% for a small root JSON alone.
				scope.parse( data, resourcePath, function ( gltf: any) {

					onLoad( gltf )

					scope.manager.itemEnd( url )

				}, _onError, trackProgress )
	 ...
	 * @return {GLTFLoader} A reference to this loader.
	 */
	register( callback: ( parser: GLTFParser ) => any ): this {
	 ...
	 * @return {GLTFLoader} A reference to this loader.
	 */
	unregister( callback: ( parser: GLTFParser ) => any ): this {
	 ...
	 * @param {onErrorCallback} onError - Executed when errors occur.
	 */
	parse( data: ArrayBuffer | string, path: string, onLoad: ( gltf: GLTF ) => void, onError?: ( event: ErrorEvent | Error ) => void, trackProgress?: TrackProgress ): void {

		let json: any
		const extensions: Record<string, any> = {}
		const plugins: Record<string, GLTFLoaderPlugin> = {}
		...
		const resourcePath = path || this.resourcePath || ''
		...
		if ( trackProgress && json.buffers ) {
			...
			for ( let i = 0; i < json.buffers.length; i ++ ) {
				...
				trackProgress( LoaderUtils.resolveURL( buffer.uri, resourcePath ) )(
...
function getMaterialExtension( parser: any, materialIndex: any, extensionName: any) {
...
class GLTFLightsExtension {
	...
	getDependency( type: any, index: any) {
	...
	createNodeAttachment( nodeIndex: any) {
		...
		return this._loadLight( lightIndex ).then( function ( light: any) {
...
class GLTFMaterialsEmissiveStrengthExtension {
	...
	extendMaterialParams( materialIndex: any, materialParams: any) {
...
class GLTFMaterialsClearcoatExtension {
	...
	getMaterialType( materialIndex: any) {
...
class GLTFMeshoptCompression {
	...
	constructor( parser: any, name: any) {
	...
	loadBufferView( index: any) {
		...
		if ( bufferView.extensions && bufferView.extensions[ this.name ] ) {
			...
			return buffer.then( function ( res: any) {
				...
				if ( decoder.decodeGltfBufferAsync ) {
					...
					return decoder.decodeGltfBufferAsync( count, stride, source, extensionDef.mode, extensionDef.filter ).then( function ( res: any) {
...
class GLTFMeshGpuInstancing {
	...
	createNodeMesh( nodeIndex: any) {
		...
		for ( const key in attributesDef ) {
			...
			pending.push( this.parser.getDependency( 'accessor', attributesDef[ key ] ).then( ( accessor: any ) => {
...
class GLTFDracoMeshCompressionExtension {
	...
	decodePrimitive( primitive: any, parser: any) {
		...
		return parser.getDependency( 'bufferView', bufferViewIndex ).then( function ( bufferView: any) {
...
class GLTFTextureTransformExtension {
	...
	extendTexture( texture: any, transform: any) {
...
const WEBGL_FILTERS: Record<number, any> = {
	9728: NearestFilter,
	9729: LinearFilter,
...
const WEBGL_WRAPPINGS: Record<number, any> = {
...
const INTERPOLATION: Record<string, any> = {
 ...
 * @return {Material}
 */
function createDefaultMaterial( cache: any) {
...
function addUnknownExtensionsToUserData( knownExtensions: any, object: any, objectDef: any) {
 ...
 */
function assignExtrasToUserData( object: any, gltfDef: any) {
 ...
 */
function addMorphTargets( geometry: any, targets: any, parser: any) {
 ...
 */
function updateMorphTargets( mesh: any, meshDef: any) {
...
function createPrimitiveKey( primitiveDef: any) {
...
function getImageURIMimeType( uri: any) {
...
export class GLTFParser {
	...
	cache: { get: ( key: string ) => any; add: ( key: string, object: any ) => void; remove: ( key: string ) => void; removeAll: () => void }
	associations: Map<Object3D | Material | Texture, GLTFReference>
	primitiveCache: Record<string, any>
	nodeCache: Record<string, any>
	meshCache: { refs: Record<string, any>; uses: Record<string, any> }
	cameraCache: { refs: Record<string, any>; uses: Record<string, any> }
	lightCache: { refs: Record<string, any>; uses: Record<string, any> }
	sourceCache: Record<string, any>
	textureCache: Record<string, any>
	nodeNamesUsed: Record<string, number>
	...
	parse( onLoad: any, onError: any) {
		...
		Promise.all( this._invokeAll( function ( ext: any) {
			...
			return Promise.all( parser._invokeAll( function ( ext: any) {
	 ...
	 */
	_addNodeRef( cache: any, index: any) {
	 ...
	 * @return {Object}
	 */
	_getNodeRef( cache: any, index: any, object: any) {
		...
		const updateMappings = ( original: any, clone: any ) => {
	...
	_invokeOne( func: any) {
		...
		const extensions: any[] = Object.values( this.plugins )
	...
	getDependency( type: any, index: any) {
		...
		if ( ! dependency ) {
			...
			switch ( type ) {
				...
				case 'node':
					dependency = this._invokeOne( function ( ext: any) {
				...
				default:
					dependency = this._invokeOne( ( ext: any ) => {
	...
	getDependencies( type: any) {
		...
		if ( ! dependencies ) {
			...
			dependencies = Promise.all( defs.map( function ( def: any, index: any) {
	...
	loadBuffer( bufferIndex: any) {
		...
		return new Promise( function ( resolve: any, reject: any) {
			...
			const bufferUrl = LoaderUtils.resolveURL( bufferDef.uri, options.path )

			loader.load( bufferUrl, resolve, options.trackProgress ? options.trackProgress( bufferUrl ) : undefined, function () {
	 ...
	 * @return {Promise<ArrayBuffer>}
	 */
	loadBufferView( bufferViewIndex: any) {
	 ...
	 */
	loadAccessor( accessorIndex: any) {
		...
		return Promise.all( pendingBufferViews ).then( function ( bufferViews: any) {
	...
	loadTextureImage( textureIndex: any, sourceIndex: any, loader: any) {
		...
		const promise = this.loadImageSource( sourceIndex, loader ).then( function ( texture: any) {
	...
	loadImageSource( sourceIndex: any, loader: TextureLoader | ImageBitmapLoader | Loader ) {
		...
		if ( this.sourceCache[ sourceIndex ] !== undefined ) {
			...
			return this.sourceCache[ sourceIndex ].then( ( texture: Texture ) => texture.clone() )
		...
		if ( sourceDef.bufferView !== undefined ) {
			...
			sourceURI = parser.getDependency( 'bufferView', sourceDef.bufferView ).then( function ( bufferView: any) {
		...
		const promise = Promise.resolve( sourceURI ).then( function ( sourceURI: any) {
	 ...
	 * @return {Promise<Texture>}
	 */
	assignTexture( materialParams: any, mapName: any, mapDef: any, colorSpace?: string ) {

		const parser = this

		return this.getDependency( 'texture', mapDef.index ).then( function ( texture: any) {
	 ...
	 */
	loadMaterial( materialIndex: any) {
		...
		if ( materialExtensions[ EXTENSIONS.KHR_MATERIALS_UNLIT ] ) {
			...
			materialType = this._invokeOne( function ( ext: any) {

				return ext.getMaterialType && ext.getMaterialType( materialIndex )

			} )

			pending.push( Promise.all( this._invokeAll( function ( ext: any) {
	 ...
	 * @return {string}
	 */
	createUniqueName( originalName: any) {
	...
	loadGeometries( primitives: any) {
		...
		function createDracoPrimitive( primitive: any) {

			return extensions[ EXTENSIONS.KHR_DRACO_MESH_COMPRESSION ]
				.decodePrimitive( primitive, parser )
				.then( function ( geometry: any) {
	...
	loadMesh( meshIndex: any) {
		...
		return Promise.all( pending ).then( function ( results: any) {
	...
	loadCamera( cameraIndex: any) {
		...
		if ( cameraDef.name && camera ) camera.name = this.createUniqueName( cameraDef.name )
	 ...
	 */
	loadAnimation( animationIndex: any) {
	...
	createNodeMesh( nodeIndex: any) {
		...
		return parser.getDependency( 'mesh', nodeDef.mesh ).then( function ( mesh: any) {
	...
	loadNode( nodeIndex: any) {
		...
		return Promise.all( [
			...
			if ( skeleton !== null ) {
				...
				node.traverse( function ( mesh: any) {
	...
	_loadNodeShallow( nodeIndex: any) {
		...
		const meshPromise = parser._invokeOne( function ( ext: any) {
		...
		if ( nodeDef.camera !== undefined ) {
			...
			pending.push( parser.getDependency( 'camera', nodeDef.camera ).then( function ( camera: any) {
		...
		parser._invokeAll( function ( ext: any) {

			return ext.createNodeAttachment && ext.createNodeAttachment( nodeIndex )

		} ).forEach( function ( promise: any) {

			pending.push( promise )

		} )

		this.nodeCache[ nodeIndex ] = Promise.all( pending ).then( function ( objects: any) {
	...
	loadScene( sceneIndex: any) {
		...
		return Promise.all( pending ).then( function ( nodes: any) {
	...
	_createAnimationTracks( node: any, inputAccessor: any, outputAccessor: any, sampler: any, target: any) {
		...
		function collectMorphTargets( object: any) {
	...
	_getArrayFromAccessor( accessor: any) {
	...
	_createCubicSplineTrackInterpolant( track: any) {
 ...
 * @param {GLTFParser} parser
 */
function computeBounds( geometry: any, primitiveDef: any, parser: any) {
 ...
 * @return {Promise<BufferGeometry>}
 */
function addPrimitiveAttributes( geometry: any, primitiveDef: any, parser: any) {

	const attributes = primitiveDef.attributes

	const pending = []

	function assignAttributeAccessor( accessorIndex: any, attributeName: any) {

		return parser.getDependency( 'accessor', accessorIndex )
			.then( function ( accessor: any) {
	...
	if ( primitiveDef.indices !== undefined && ! geometry.index ) {
		...
		const accessor = parser.getDependency( 'accessor', primitiveDef.indices ).then( function ( accessor: any) {
