...
export class GLTFGoogleTiltBrushTechniquesExtension {
    ...
    beforeRoot(): null | undefined {
        ...
        json.materials.map((material: any, index: number) => {
            ...
            //MainTex
            const mainTexIndex = extensionsDef.GOOGLE_tilt_brush_techniques.values.MainTex
            if(mainTexIndex !== undefined) {
                json.materials[index].pbrMetallicRoughness.baseColorTexture.index = mainTexIndex
            }

            //BumpMap
            const bumpMapIndex = extensionsDef.GOOGLE_tilt_brush_techniques.values.BumpMap
