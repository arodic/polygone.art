...
export type UniformSeedEntry = { value?: any };
...
export type LegacyUniformMap = Record<string, { value: any }>;
...
export function hasNodeBrush(brushName: string): boolean {
	return NODE_BRUSH_NAMES.has(brushName);
}

function makeUniform(value: any): TiltUniformEntry {
...
export function createTiltUniformBag(seed: UniformSeed = {}): TiltUniformBag {
	const u: TiltUniformBag = {};

	const ensureVec4 = (key: string, fallback: Vector4 = new Vector4()) => {
	...
	const ensureFloat = (key: string, fallback = 0) => {
		const value = typeof seed[key]?.value === 'number' ? seed[key].value : fallback;
		u[key] = makeUniform(value);
	};

	const ensureMat4 = (key: string) => {
	...
	{
		const texel =
			seed.u_AlphaMask_TexelSize?.value?.clone?.() ??
	...
	{
		const base =
			seed.u_BaseColorFactor?.value?.clone?.() ??
...
export function toLegacyUniforms(tiltUniforms: TiltUniformBag, textures: Record<string, any> = {}): LegacyUniformMap {
	const uniforms: LegacyUniformMap = {};
	for (const [key, entry] of Object.entries(tiltUniforms) as [string, TiltUniformEntry][]) {
 ...
 */
export const surfaceShaderSpecularGloss = Fn(([
	normal,
	lightDir,
	eyeDir,
	lightColor,
	...
	const oneMinusSpecularIntensity = float(1).sub(
		clamp(max(max(specularColor.x, specularColor.y), specularColor.z), float(0), float(1)),
	);
	...
	const withSpec = lightColor.mul(
		diffuseColor.mul(diffuseTerm).add(fresnelColor.mul(specularTerm)),
	);
 ...
 */
export const surfaceShaderMetallicRoughness = Fn(([
	normal,
	lightDir,
	eyeDir,
	lightColor,
	...
	const fresnelT = pow5(float(1).sub(LdotH));
	const fresnelColor = specularColor.add(vec3(1, 1, 1).sub(specularColor).mul(fresnelT));
	return lightColor.mul(
		diffuseColor.mul(diffuseTerm).add(fresnelColor.mul(specularTerm)),
	);
});

export const diffuseLighting = Fn(([
	normal,
	lightDir0,
	lightDir1,
	lightColor0,
 ...
 */
export const surfaceLighting = Fn(([
	normal,
	lightDir0,
	lightDir1,
	eyeDir,
 ...
 */
export const surfaceLightingDoubleSide = Fn(([
	normal,
	lightDir0,
	lightDir1,
	eyeDir,
 ...
 */
export const surfaceLightingNoFacingFlip = Fn(([
	normal,
	lightDir0,
	lightDir1,
	eyeDir,
...
const snoise2D = Fn(([v]) => {
	...
	const p = permute_v3(
		permute_v3(ii.y.add(vec3(0.0, i1.y, 1.0))).add(ii.x.add(vec3(0.0, i1.x, 1.0))),
	);
	const m0 = max(
		float(0.5).sub(vec3(dot(x0, x0), dot(x12xy, x12xy), dot(x12.zw, x12.zw))),
		float(0),
	);
...
const snoise3D = Fn(([v]) =>
	vec3(snoise2D(vec2(v.x, v.y)), snoise2D(vec2(v.y, v.z)), snoise2D(vec2(v.z, v.x))),
);
 ...
 */
export const bubblesCurlDisplacement = Fn(
	([worldCenter, timeVec, scrollRate, jitterIntensity, jitterFrequency]) => {
		const seed = worldCenter.mul(10.0);
		const t = timeVec.y.mul(scrollRate).add(1.0);
		const jitter = vec3(
			sin(t.add(timeVec.y).add(seed.z.mul(jitterFrequency))),
			cos(t.mul(1.2).add(timeVec.y).add(seed.x.mul(jitterFrequency))),
			cos(t.add(timeVec.y).add(seed.x.mul(jitterFrequency))),
		).mul(jitterIntensity);
 ...
 */
export const bubblesParticlePositionLocal = Fn(
	([timeVec, scrollRate, jitterIntensity, jitterFrequency]) => {
		const base = particlePositionLocal();
		const center = attribute('a_normal', 'vec3');
		const worldCenter = modelWorldMatrix.mul(vec4(center, 1)).xyz;
		const disp = bubblesCurlDisplacement(
			worldCenter,
			timeVec,
			scrollRate,
			jitterIntensity,
 ...
 */
export const bubbleWandPositionLocal = Fn(
	([timeVec, scrollRate, jitterIntensity, jitterFrequency, isNewExporter]) => {
		const pos = brushPositionAttr();
		const uv0 = brushUv3();
		...
		const radius = select(
			useBaked,
			select(bakedRadius.greaterThan(1e-6), bakedRadius, uv0.z),
			uv0.z,
		);
		...
		const jitterX = sin(
			t.add(timeVec.y).add(vertex.z.mul(jitterFrequency)),
		).mul(jitterIntensity);
		const jitterZ = cos(
			t.add(timeVec.y).add(vertex.x.mul(jitterFrequency)),
		).mul(jitterIntensity);
		const jitterY = cos(
			t.mul(1.2).add(timeVec.y).add(vertex.x.mul(jitterFrequency)),
		).mul(jitterIntensity);
...
export const hyperGridPositionLocal = Fn(([isNewExporter]) => {
	...
	const outWorld = select(
		isNewExporter.lessThan(0.5),
		select(size.greaterThan(1e-6), quantized, worldPos),
		worldPos,
	);
...
export const electricityPositionLocal = Fn(([timeVec, displacementIntensity]) => {
	...
	const displaced = tapered.add(
		widthiness.mul(disp).mul(displacementIntensity).mul(envelopePow),
	);
...
export function lightDirVaryings(u: TiltUniformBag) {
	// mat3(matrix) * vec3(0,0,1) == third column of the upper 3x3
	const ld0 = varying(
		u.u_SceneLight_0_matrix.node.mul(vec4(0, 0, 1, 0)).xyz,
		'v_tb_light0',
	);
	const ld1 = varying(
		u.u_SceneLight_1_matrix.node.mul(vec4(0, 0, 1, 0)).xyz,
		'v_tb_light1',
	);
	return { ld0, ld1 };
}
...
export function viewPositionVarying(positionNode: any = positionLocal) {
	return varying(modelViewMatrix.mul(vec4(positionNode, 1)).xyz, 'v_tb_viewpos');
}

export function fogCoordVarying(positionNode: any = positionLocal) {
	const viewPos = modelViewMatrix.mul(vec4(positionNode, 1));
	return varying(viewPos.z, 'f_fog_coord');
}

export function applyCommonMaterialProps(material: BrushMaterialLike, params: BrushMaterialProps = {}): BrushMaterialLike {
	...
	if (params.blendSrc !== undefined) material.blendSrc = params.blendSrc;
	if (params.blendDst !== undefined) material.blendDst = params.blendDst;
	if (params.blendEquation !== undefined) material.blendEquation = params.blendEquation;
	if (params.blendSrcAlpha !== undefined) material.blendSrcAlpha = params.blendSrcAlpha;
	if (params.blendDstAlpha !== undefined) material.blendDstAlpha = params.blendDstAlpha;
	if (params.blendEquationAlpha !== undefined) material.blendEquationAlpha = params.blendEquationAlpha;
	if (params.alphaToCoverage !== undefined) material.alphaToCoverage = params.alphaToCoverage;
	return material;
}
