...
export class PresentationLoader {
    async load(jsonUrl) {
        try {
            ...
            if (res.ok) {
                const parsed = this.parsePresentationCamera((await res.json()));
                console.log(parsed);
                if (parsed)
                    return parsed;
            }
    ...
    parsePresentationCamera(raw) {
        ...
        if (!Array.isArray(rotation) || rotation.length < 4)
            return null;
        const perspective = cam.perspective && typeof cam.perspective === 'object'
            ? cam.perspective
            : cam;
