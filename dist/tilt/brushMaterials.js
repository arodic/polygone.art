...
import { Vector3, Vector4, FrontSide } from 'three';
import { Discard, Fn as tslFn, If, abs, attribute, ceil, clamp, cos, cross, cameraPosition, dFdx, dFdy, dot, exp, float, floor, fract, frontFacing, interleavedGradientNoise, length, log2, max, min, mix, mod, modelWorldMatrix, modelWorldMatrixInverse, normalize, pow, select, sin, screenCoordinate, sqrt, step, texture, textureSize, varying, vec2, vec3, vec4, wgslFn, } from 'three/tsl';
import { BLEND, applyCommonMaterialProps, applyFog, bloomColor, brushColor, brushNormalAttr, brushPositionAttr, brushQuadIndex, brushTimestamp, brushUv2, brushUv4, bubbleWandPositionLocal, bubblesParticlePositionLocal, createTiltUniformBag, diffuseLighting, discoPositionLocal, doubleTaperedPositionLocal, fogCoordVarying, hasNodeBrush, hyperGridPositionLocal, electricityPositionLocal, lambertShader, lightWirePositionLocal, lightDirVaryings, particlePositionLocal, perturbNormal, perturbNormalFacing, rainInflatePositionLocal, smokeParticlePositionLocal, surfaceLighting, surfaceLightingDoubleSide, surfaceShaderMetallicRoughness, surfaceShaderSpecularGloss, shShaderWithSpec, toLegacyUniforms, viewNormalVarying, viewPositionVarying, viewTBNVaryings, waveformParticlesPositionLocal, } from './brushCommon.js';
const Fn = tslFn;
function sampleMain(tex, uvNode) {
    return texture(tex, uvNode);
}
function attachUniforms(material, tiltUniforms, textures) {
...
function buildCutoffUnlit({ name, mainTex, cutoff, fog = true, blend = BLEND.opaque }) {
...
function buildCutoffUnlitNoVertexAlpha({ name, mainTex, cutoff, blend = BLEND.opaque }) {
    const u = createTiltUniformBag({ u_Cutoff: { value: cutoff } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, blend);
    material.name = `material_${name}`;
...
function buildSolidUnlit({ name, blend = BLEND.opaqueFront, doubleTaper = false }) {
 ...
 */
function buildHyperGrid({ mainTex, tintColor }) {
    const u = createTiltUniformBag({
 ...
 */
function buildElectricity({ emissionGain, displacementIntensity }) {
    const u = createTiltUniformBag({
    ...
    material.positionNode = electricityPositionLocal(u.u_time.node, u.u_DisplacementIntensity.node);
    const uv0 = brushUv2();
...
function buildDiffuseLit({ name, mainTex = null, cutoff = null, blend = BLEND.opaque, a2c = false, multiplyVertexAlpha = false, }) {
    const seed = {};
    if (cutoff != null)
        seed.u_Cutoff = { value: cutoff };
    if (a2c)
        seed.u_A2CEnabled = { value: 1 };
    ...
    const fogCoord = fogCoordVarying();
    const vUv = mainTex ? varying(brushUv2(), 'v_tb_uv') : null;
    material.fragmentNode = Fn(() => {
        if (mainTex && cutoff != null && !a2c) {
            let brushMask = sampleMain(mainTex, vUv).w;
            if (multiplyVertexAlpha)
                brushMask = brushMask.mul(vColor.a);
            If(brushMask.lessThanEqual(u.u_Cutoff.node), () => {
                Discard();
            });
        }
        const lit = diffuseLighting(vNormal, ld0, ld1, u.u_SceneLight_0_color.node.rgb, u.u_SceneLight_1_color.node.rgb, u.u_ambient_light_color.node.rgb, vColor.rgb);
 ...
 */
function buildLeakyPen({ mainTex, cutoff, mainTexST }) {
    const u = createTiltUniformBag({ u_Cutoff: { value: cutoff } });
    const material = new MeshBasicNodeMaterial();
    ...
    const { ld0, ld1 } = lightDirVaryings(u);
    const fogCoord = fogCoordVarying();
    const vUv0 = varying(brushUv2().mul(vec2(mainST.x, mainST.y)).add(vec2(mainST.z, mainST.w)), 'v_tb_uv0');
    material.fragmentNode = Fn(() => {
 ...
 */
function buildLoftedHueShift({ mainTex }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.opaqueFront);
...
function buildQuillA2C({ name, cutoff, a2cEnabled, ditherStrength, orderedDither, alphaBias, alphaPower, }) {
    const u = createTiltUniformBag({
    ...
    material.fragmentNode = Fn(() => {
        ...
        const alpha = clamp(pow(biasedAlpha, max(u.u_AlphaPower.node, float(0.0001))), float(0), float(1));
        ...
        const adjustedAlpha = clamp(alpha.add(dither.sub(0.5).mul(ditherAmount)), float(0), float(1));
        If(u.u_A2CEnabled.node.greaterThanEqual(0.5).and(adjustedAlpha.lessThan(0.01)), () => {
            Discard();
        });
        const outAlpha = select(u.u_A2CEnabled.node.lessThan(0.5), float(1), adjustedAlpha);
...
function buildFacetedTube({ colorX, colorY, colorZ }) {
    const u = createTiltUniformBag({
    ...
    const vColor = varying(brushColor(), 'v_tb_color');
    const vWorldPos = varying(modelWorldMatrix.mul(vec4(brushPositionAttr(), 1)).xyz, 'v_tb_worldpos');
    material.fragmentNode = Fn(() => {
...
function buildTaperedHueShift({ mainTex, cutoff }) {
    const u = createTiltUniformBag({ u_Cutoff: { value: cutoff } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.opaqueFront);
...
function buildUnlitMask({ name, mainTex, blend }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, blend);
    material.name = `material_${name}`;
...
function quillParams(name, uniforms) {
 ...
 */
function buildIcing({ mainTex, bumpMap, cutoff, shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
    ...
    material.fragmentNode = Fn(() => {
        ...
        const lit = surfaceLighting(bumped, ld0, ld1, vPos.negate(), // eyeDir = -normalize(v_position)
        u.u_SceneLight_0_color.node.rgb, u.u_SceneLight_1_color.node.rgb, u.u_ambient_light_color.node.rgb, vColor.rgb, u.u_SpecColor.node, u.u_Shininess.node);
        const rgb = applyFog(lit, fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
        If(brushMask.lessThanEqual(u.u_Cutoff.node), () => {
            Discard();
        });
    ...
    const textures = {};
    if (mainTex)
        textures.u_MainTex = mainTex;
    if (bump)
        textures.u_BumpMap = bump;
    attachUniforms(material, u, textures);
    return material;
}
function buildHighlighter({ mainTex, cutoff }) {
    const u = createTiltUniformBag({ u_Cutoff: { value: cutoff } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
...
function buildSoftHighlighter({ mainTex }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
...
function buildFire({ mainTex, emissionGain }) {
    ...
    material.fragmentNode = Fn(() => {
        ...
        const tex = sampleMain(mainTex, vUv.sub(vec2(0, 0)).sub(vec2(displacement.mul(displacementIntensity), 0)));
...
function buildLight({ mainTex, emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additiveOneOne);
 ...
 */
function buildWaveform({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    material.fragmentNode = Fn(() => {
        ...
        const proceduralLine = clamp(float(1).sub(pinch.mul(abs(vUv.y.sub(0.5).sub(waveform.mul(envelope))))), float(0), float(1));
 ...
 */
function buildDoubleTaperedFlat({ shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
    ...
    material.fragmentNode = Fn(() => {
        const lit = surfaceLightingDoubleSide(vNormal, ld0, ld1, vPos.negate(), u.u_SceneLight_0_color.node.rgb, u.u_SceneLight_1_color.node.rgb, u.u_ambient_light_color.node.rgb, vColor.rgb, u.u_SpecColor.node, u.u_Shininess.node);
        const rgb = applyFog(lit, fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
        return vec4(rgb, 1);
    })();
    attachUniforms(material, u, {});
 ...
 */
function buildShinyHull({ name = 'ShinyHull', shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
    ...
    const fogCoord = fogCoordVarying();
    material.fragmentNode = Fn(() => {
        const lit = surfaceLighting(vNormal, ld0, ld1, vPos.negate(), u.u_SceneLight_0_color.node.rgb, u.u_SceneLight_1_color.node.rgb, u.u_ambient_light_color.node.rgb, vColor.rgb, u.u_SpecColor.node, u.u_Shininess.node);
        const rgb = applyFog(lit, fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
        return vec4(rgb, 1);
    })();
    attachUniforms(material, u, {});
 ...
 */
function buildLeaves({ mainTex, cutoff, shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
 ...
 */
function buildStars({ mainTex, sparkleRate }) {
    ...
    const brightness = float(800).mul(pow(abs(sin(u.u_time.node.y.mul(u.u_SparkleRate.node).add(phase))), float(20)));
 ...
 */
function buildPetal({ shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
 ...
 */
function buildWigglyGraphite({ mainTex, cutoff }) {
    const u = createTiltUniformBag({ u_Cutoff: { value: cutoff } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.opaque);
    ...
    material.fragmentNode = Fn(() => {
        ...
        const lit = surfaceLightingDoubleSide(vNormal, ld0, ld1, vPos.negate(), u.u_SceneLight_0_color.node.rgb, u.u_SceneLight_1_color.node.rgb, u.u_ambient_light_color.node.rgb, vColor.rgb, vec3(0, 0, 0), float(0));
        const rgb = applyFog(lit, fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
        return vec4(rgb, 1);
    })();
    attachUniforms(material, u, { u_MainTex: mainTex });
 ...
 */
function buildCelVinyl({ mainTex, cutoff }) {
    const u = createTiltUniformBag({ u_Cutoff: { value: cutoff } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.opaque);
 ...
 */
function buildNeonPulse({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additiveOneOne);
    ...
    material.fragmentNode = Fn(() => {
        ...
        const lightOut0 = surfaceShaderSpecularGloss(nFacing, ld0n, ed, u.u_SceneLight_0_color.node.rgb, diffuse, spec, float(0.8));
        const lightOut1 = shShaderWithSpec(nFacing, ld1n, u.u_SceneLight_1_color.node.rgb, diffuse, spec);
 ...
 */
function buildLightWire({ shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
    ...
    material.fragmentNode = Fn(() => {
        ...
        let specularColor = vec3(0.3, 0.3, 0.3).sub(lights.mul(vec3(0.15, 0.15, 0.15)));
        const smoothness = float(0.3).sub(lights.mul(0.3));
        let color = vColor;
        const colorindex = floor(mod(vUv.x.mul(2).add(0.5), float(3)));
        const tinted = select(colorindex.equal(0), color.rgb.mul(vec3(0.2, 0.2, 1)), select(colorindex.equal(1), color.rgb.mul(vec3(1, 0.2, 0.2)), color.rgb.mul(vec3(0.2, 1, 0.2))));
        ...
        let diffuseColor = float(1).sub(lights).mul(color.rgb).mul(0.2);
        diffuseColor = diffuseColor.mul(border);
        specularColor = specularColor.mul(border);
        const n = normalize(vNormal);
        const nFacing = select(frontFacing, n, n.negate());
        const lit = surfaceShaderSpecularGloss(nFacing, normalize(ld0), normalize(vPos.negate()), u.u_SceneLight_0_color.node.rgb, diffuseColor, specularColor, smoothness)
            .add(shShaderWithSpec(nFacing, normalize(ld1), u.u_SceneLight_1_color.node.rgb, diffuseColor, u.u_SpecColor.node))
 ...
 */
function buildDiamondHull({ mainTex }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additiveOneOne);
    ...
    const vPos = viewPositionVarying();
    const vWorldPos = varying(modelWorldMatrix.mul(vec4(brushPositionAttr(), 1)).xyz, 'v_tb_worldPos');
    const vWorldNormal = varying(normalize(modelWorldMatrix.mul(vec4(brushNormalAttr(), 0)).xyz), 'v_tb_worldNormal');
    const { ld0, ld1 } = lightDirVaryings(u);
    material.fragmentNode = Fn(() => {
        ...
        let rim = float(1).sub(abs(dot(viewDirWorld, normalWorld)));
        rim = rim.mul(float(1).sub(pow(rim, float(5))));
        const ndotv = abs(dot(viewDirWorld, normalWorld));
        rim = mix(rim, float(150), float(1).sub(clamp(ndotv.div(0.1), float(0), float(1))));
        ...
        const reflectAt = (lambda) => {
            const phi = float(2)
                .mul(PI)
                .div(lambda)
                .mul(float(2).mul(n1).mul(thick).mul(cos1))
                .add(delta);
            const tsInt = betaS.mul(betaS).div(alphaS.mul(alphaS).sub(float(2).mul(alphaS).mul(cos(phi))).add(1));
            const tpInt = betaP.mul(betaP).div(alphaP.mul(alphaP).sub(float(2).mul(alphaP).mul(cos(phi))).add(1));
        ...
        const lit = surfaceShaderSpecularGloss(nFacing, normalize(ld0), eyeDir, u.u_SceneLight_0_color.node.rgb, albedo, specColor, shininess)
            .add(shShaderWithSpec(nFacing, normalize(ld1), u.u_SceneLight_1_color.node.rgb, albedo, specColor))
 ...
 */
function buildSurfaceBumpCutoff({ name, mainTex, bumpMap, cutoff, shininess, specColor, invertFacing = true, blend = BLEND.opaque, useCutoff = true, }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    ...
    material.fragmentNode = Fn(() => {
        ...
        if (invertFacing) {
            const bumped = perturbNormal(vTangent, vBitangent, vNormal, vUv, bump);
            lit = surfaceLightingDoubleSide(bumped, ld0, ld1, vPos.negate(), u.u_SceneLight_0_color.node.rgb, u.u_SceneLight_1_color.node.rgb, u.u_ambient_light_color.node.rgb, vColor.rgb, u.u_SpecColor.node, u.u_Shininess.node);
        }
        else {
            ...
            const nFacing = normalize(normalize(t0).mul(tn.x).add(normalize(b0).mul(tn.y)).add(normalize(n0).mul(tn.z)));
            const ed = normalize(vPos.negate());
            const ld0n = normalize(ld0);
            const ld1n = normalize(ld1);
            const lightOut0 = surfaceShaderSpecularGloss(nFacing, ld0n, ed, u.u_SceneLight_0_color.node.rgb, vColor.rgb, u.u_SpecColor.node, u.u_Shininess.node);
            const lightOut1 = shShaderWithSpec(nFacing, ld1n, u.u_SceneLight_1_color.node.rgb, vColor.rgb, u.u_SpecColor.node);
 ...
 */
function buildDisneyCutoff({ name, mainTex, cutoff, shininess, specColor, maskVertexAlpha = false, blend = BLEND.opaque, }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    ...
    const fogCoord = fogCoordVarying();
    material.fragmentNode = Fn(() => {
        let brushMask = mainTex ? sampleMain(mainTex, vUv).w : float(1);
        if (maskVertexAlpha) {
            brushMask = brushMask.mul(vColor.a);
        }
        If(brushMask.lessThanEqual(u.u_Cutoff.node), () => {
...
function buildVelvetInk({ mainTex }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    material.fragmentNode = Fn(() => {
        ...
        const rgb = applyFog(brushTex.a.mul(vColor.rgb), fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
        return vec4(rgb, 1);
    })();
    attachUniforms(material, u, { u_MainTex: mainTex });
    return material;
 ...
 */
function buildRainbow({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    material.fragmentNode = Fn(() => {
        ...
        const band = select(scrolled.equal(0), vec3(1, 0, 0), select(scrolled.equal(1), vec3(0.7, 0.3, 0), select(scrolled.equal(2), vec3(0, 1, 0), select(scrolled.equal(3), vec3(0, 0.2, 1), vec3(0.4, 0, 1.2)))));
 ...
 */
function buildChromaticWave({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    material.fragmentNode = Fn(() => {
        ...
        ty = mod(ty.add(vUv.x), float(1));
        const wr = float(0.15).mul(sin(float(-20).mul(vColor.r).mul(timeW).add(vUv.x.mul(100).mul(vColor.r))));
        const wg = float(0.15).mul(sin(float(-30).mul(vColor.g).mul(timeW).add(vUv.x.mul(100).mul(vColor.g))));
        const wb = float(0.15).mul(sin(float(-40).mul(vColor.b).mul(timeW).add(vUv.x.mul(100).mul(vColor.b))));
 ...
 */
function buildHypercolor({ mainTex, bumpMap, cutoff, shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    ...
    material.fragmentNode = Fn(() => {
        ...
        const eyeDir = normalize(vPos.negate());
        const lightOut0 = surfaceShaderSpecularGloss(n, normalize(ld0), eyeDir, u.u_SceneLight_0_color.node.rgb, diffuseColor, specularColor, u.u_Shininess.node);
        const lightOut1 = shShaderWithSpec(n, normalize(ld1), u.u_SceneLight_1_color.node.rgb, diffuseColor, u.u_SpecColor.node);
        const ambientOut = diffuseColor.mul(u.u_ambient_light_color.node.rgb);
        const rgb = applyFog(lightOut0.add(lightOut1).add(ambientOut), fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
 ...
 */
function buildDisco({ shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
    ...
    material.fragmentNode = Fn(() => {
        ...
        const eyeDir = normalize(vPos.negate());
        const lightOut0 = surfaceShaderSpecularGloss(faceted, normalize(ld0), eyeDir, u.u_SceneLight_0_color.node.rgb, diffuseColor, specularColor, u.u_Shininess.node);
        const lightOut1 = shShaderWithSpec(smoothN, normalize(ld1), u.u_SceneLight_1_color.node.rgb, diffuseColor, u.u_SpecColor.node);
        const ambientOut = diffuseColor.mul(u.u_ambient_light_color.node.rgb);
        const fake = specularColor.mul(pow(abs(dot(faceted, vec3(0, 1, 0))), float(10)).mul(20));
 ...
 */
function buildComet({ mainTex, alphaMask, speed, alphaMaskTexelSize }) {
    const u = createTiltUniformBag({
        u_Speed: { value: speed },
...
function buildParticleBillboardMaterial({ name, fragmentFn, blend, seed = {}, mainTex }) {
    const u = createTiltUniformBag(seed);
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, blend);
    material.name = `material_${name}`;
...
function buildSmoke({ mainTex, scrollRate }) {
 ...
 */
function buildBubbles({ mainTex, scrollRate, scrollJitterIntensity, scrollJitterFrequency, }) {
    const u = createTiltUniformBag({
    ...
    material.positionNode = bubblesParticlePositionLocal(u.u_time.node, u.u_ScrollRate.node, u.u_ScrollJitterIntensity.node, u.u_ScrollJitterFrequency.node);
 ...
 */
function buildStreamers({ mainTex, emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    material.fragmentNode = Fn(() => {
        ...
        const rowRand = fract(sin(dot(vec2(rowId, rowId), vec2(12.9898, 78.233).mul(2))).mul(43758.5453));
        ...
        let tex = sampleMain(mainTex, uvs);
 ...
 */
function buildPlasma({ mainTex, mainTexST }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    const vColor = varying(brushColor(), 'v_tb_color');
    const vUv = varying(brushUv2().mul(vec2(st.x, st.y)).add(vec2(st.z, st.w)), 'v_tb_uv');
    material.fragmentNode = Fn(() => {
        ...
        let tmp = M.mul(A).mul(vUv.x).sub(bRate.mul(timeY));
        tmp = abs(fract(tmp).sub(0.5));
        let vs = vUv.y.add(float(0.4).mul(vColor.a).mul(vec3(1, -1, 1)).mul(tmp));
        vs = clamp(mix(vs.sub(0.5).mul(4), vs, sin(float(1.570795).mul(vColor.a))), float(0), float(1));
        let tex = sampleMain(mainTex, vec2(abs(us.x), vs.x));
        ...
        const procline = float(1).sub(clamp(pow(abs(vs.sub(LINE_POS).div(LINE_WIDTH)), vec3(2, 2, 0.2)), float(0), float(1)));
        tex = tex.add(dot(procline, vec3(1, 1, 1)).mul(0.5));
        const brightness = float(0.8).mul(float(1).add(float(30).mul(pow(vec3(1, 1, 0.1).sub(vec3(vColor.a, vColor.a, vColor.a)), vec3(5, 5, 5)))));
...
function buildDots({ mainTex, emissionGain, baseGain }) {
    return buildParticleBillboardMaterial({
        ...
        fragmentFn: ({ u, vColor, vUv, mainTex: tex }) => Fn(() => {
...
function buildEmbers({ mainTex }) {
    ...
    const aColor = brushColor();
    const t = mod(u.u_time.node.y.mul(u.u_ScrollRate.node).add(aColor.a.mul(10)), float(1));
    ...
    const dispBase = modelWorldMatrix
        ...
        .xyz.mul(t);
    const jitterX = sin(t.mul(u.u_ScrollJitterFrequency.node).add(aColor.a.mul(100)).add(t2).add(worldPosBase.z)).mul(u.u_ScrollJitterIntensity.node);
    const jitterY = mod(aColor.a.mul(100), float(1))
        .sub(0.5)
        .mul(u.u_ScrollDistance.node.y)
        .mul(t);
    const jitterZ = cos(t.mul(u.u_ScrollJitterFrequency.node).add(aColor.a.mul(100)).add(t2).add(worldPosBase.x)).mul(u.u_ScrollJitterIntensity.node);
...
function buildSnow({ mainTex, scrollRate, scrollDistance, jitterIntensity, jitterFrequency }) {
    const u = createTiltUniformBag({
        u_ScrollRate: { value: scrollRate },
    ...
    const jitterX = sin(t.mul(u.u_ScrollJitterFrequency.node).add(t2)).mul(u.u_ScrollJitterIntensity.node);
    const jitterZ = cos(t.mul(u.u_ScrollJitterFrequency.node).mul(0.5).add(t2)).mul(u.u_ScrollJitterIntensity.node);
 ...
 */
function buildBlocksBasic({ shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
 ...
 */
function buildBlocksGlass({ shininess, rimIntensity, rimPower, color }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
    ...
    material.fragmentNode = Fn(() => {
        ...
        const lightOut0 = surfaceShaderSpecularGloss(nFacing, ld0n, ed, u.u_SceneLight_0_color.node.rgb, diffuse, specularColor, u.u_Shininess.node);
 ...
 */
function buildBlocksGem({ shininess, rimIntensity, rimPower, frequency, jitter, color }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_RimIntensity: { value: rimIntensity },
        u_RimPower: { value: rimPower },
    ...
    material.fragmentNode = Fn(() => {
        ...
        normal = normalize(vec3(normal.x.add(dFdx(gem).mul(50)), normal.y.add(dFdy(gem).mul(50)), normal.z));
        const ld0n = normalize(ld0);
        const ed = normalize(vPos.negate());
        const diffuse = vec3(0, 0, 0);
        ...
        const lightOut0 = surfaceShaderSpecularGloss(normal, ld0n, ed, u.u_SceneLight_0_color.node.rgb, diffuse, specularColor, u.u_Shininess.node);
 ...
 */
function buildPbrTemplate({ baseColorTex, metallicFactor, roughnessFactor, baseColorFactor, }) {
    const u = createTiltUniformBag({
    ...
    material.fragmentNode = Fn(() => {
        ...
        const ld1n = normalize(ld1);
        const ed = normalize(vPos.negate());
        const lightOut0 = surfaceShaderMetallicRoughness(nFacing, ld0n, ed, u.u_SceneLight_0_color.node.rgb, albedo, u.u_MetallicFactor.node, u.u_RoughnessFactor.node);
        const lightOut1 = surfaceShaderMetallicRoughness(nFacing, ld1n, ed, u.u_SceneLight_1_color.node.rgb, albedo, u.u_MetallicFactor.node, u.u_RoughnessFactor.node);
        const ambientOut = albedo.mul(u.u_ambient_light_color.node.rgb);
        const lit = lightOut0.add(lightOut1).add(ambientOut);
        const rgb = applyFog(lit, fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
 ...
 * Each imported mesh has its own BaseColorTex / factors — do not share materials.
 */
function buildPbrTransparentTemplate({ baseColorTex, metallicFactor, roughnessFactor, baseColorFactor, cutoff, }) {
    const u = createTiltUniformBag({
        u_MetallicFactor: { value: metallicFactor },
        u_RoughnessFactor: { value: roughnessFactor },
        u_BaseColorFactor: { value: baseColorFactor },
...
function buildWind({ mainTex, speed }) {
...
function buildDrafting({ mainTex, opacity }) {
...
function buildRain({ mainTex, numSides, speed, mainTexST }) {
    const u = createTiltUniformBag({
    ...
    material.fragmentNode = Fn(() => {
        ...
        const uvs = vUv;
        let uCoord = uvs.x.mul(uScale).sub(t);
        const rowId = floor(uvs.y.mul(u.u_NumSides.node));
        const rand = abs(fract(sin(dot(vec2(rowId, rowId), vec2(12.9898, 78.233).mul(2))).mul(4550))).mul(0.7);
        ...
        const tiledUV = vec2(uCoord.mul(st.x).add(st.z), vCoord.mul(st.y).add(st.w));
        let tex = sampleMain(mainTex, tiledUV);
...
function waveformProceduralMask(uv, rawColor, timeW) {
    ...
    const pinch = float(1).sub(envelope).mul(40).add(20);
    const proceduralLine = clamp(float(1).sub(pinch.mul(abs(uv.y.sub(0.5).sub(waveform.mul(envelope))))), float(0), float(1));
    return envelope.mul(proceduralLine);
}
/** WaveformFFT: vertex-bloomed color × procedural lines (preview GLSL). */
function buildWaveformFFT({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
...
function buildWaveformTube({ mainTex }) {
    const u = createTiltUniformBag();
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
...
function buildWaveformParticles({ mainTex, opacity }) {
    const u = createTiltUniformBag({
...
function buildSpace({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    const spaceHash3 = Fn(([p]) => fract(sin(dot(p, vec3(12.9898, 78.233, 45.164))).mul(43758.5453)));
    ...
    material.fragmentNode = Fn(() => {
        ...
        const hue = select(maxC.equal(minC), float(0), select(maxC.equal(vColor.r), mod(vColor.g.sub(vColor.b).div(delta).add(select(vColor.g.lessThan(vColor.b), float(6), float(0))), float(1)), select(maxC.equal(vColor.g), vColor.b.sub(vColor.r).div(delta).add(2), vColor.r.sub(vColor.g).div(delta).add(4)).div(6)));
        ...
        let primaryA = float(0.2).mul(spaceFbm2(vUv.add(vec2(u.u_time.node.x, 0)))).mul(gain).add(gain2);
        let analog1A = float(0.2)
            .mul(spaceFbm3(vec3(vUv.x.add(12.52), vUv.y.add(12.52), u.u_time.node.x.mul(5.2))))
            .mul(gain)
            .add(gain2);
        let analog2A = float(0.2)
            .mul(spaceFbm3(vec3(vUv.x.add(6.253), vUv.y.add(6.253), u.u_time.node.x.mul(0.8))))
            .mul(gain)
            .add(gain2);
        const rfbmRand = spaceFbm2(vec2(u.u_time.node.x.add(50), vUv.x)).mul(2);
        primaryA = mix(primaryA, float(0), clamp(r.add(rfbmRand).sub(0).div(0.5), float(0), float(1)));
        ...
        const finalHue = primaryA
            ...
            .div(max(alpha, 1e-6));
        let lum = float(1).sub(r);
        let rfbm = spaceFbm2(vec2(vUv.x, u.u_time.node.x)).add(1.2).mul(0.8);
...
function buildSparks({ mainTex, emissionGain, stretchDistortionExponent, numSides, speed, }) {
    const u = createTiltUniformBag({
        u_EmissionGain: { value: emissionGain },
    ...
    material.fragmentNode = Fn(() => {
        ...
        let uCoord = uv.x.mul(uScale).sub(t);
        const rowId = floor(uv.y.mul(u.u_NumSides.node));
        const rand = abs(fract(sin(rowId.mul(12.9898 + 78.233).mul(2)).mul(50)).add(fract(sin(rowId.mul(12.9898 + 78.233).mul(2)).mul(50)))).mul(0.75);
        uCoord = uCoord.add(rand.mul(uScale));
        uCoord = mod(uCoord, uScale);
        const vCoord = uv.y.mul(u.u_NumSides.node);
        let tex = sampleMain(mainTex, vec2(uCoord, vCoord));
        tex = select(uCoord.lessThan(0), vec4(0, 0, 0, 0), tex);
        tex = select(uCoord.greaterThan(1), vec4(0, 0, 0, 0), tex);
...
function buildFairy({ emissionGain }) {
    const u = createTiltUniformBag({ u_EmissionGain: { value: emissionGain } });
    const material = new MeshBasicNodeMaterial();
    applyCommonMaterialProps(material, BLEND.additive);
    ...
    material.fragmentNode = Fn(() => {
        ...
        st = st.add(offset);
        let lum = float(1).sub(length(st));
        lum = lum.sub(max(offset.x, offset.y));
        lum = clamp(lum, float(0), float(1));
        let powpow = fairyRandom(rc).mul(2).sub(1);
...
function buildFire2({ mainTex, displaceTex, scroll1, scroll2, displacementIntensity, flameFadeMin, flameFadeMax, }) {
    const u = createTiltUniformBag({
    ...
    material.fragmentNode = Fn(() => {
        ...
        const flame1 = sampleMain(mainTex, uv.mul(0.7).add(vec2(u.u_time.node.x.mul(u.u_Scroll1.node).negate(), 0))).x;
        const flame2 = sampleMain(mainTex, vec2(uv.x, float(1).sub(uv.y)).add(vec2(u.u_time.node.x.mul(u.u_Scroll2.node).negate(), u.u_time.node.x.mul(u.u_Scroll2.node).negate().div(4)))).x;
        let flames = clamp(flame2.add(flame1).div(2), float(0), float(1));
    ...
    const textures = { u_MainTex: mainTex };
    if (displaceTex)
        textures.u_DisplaceTex = displaceTex;
    attachUniforms(material, u, textures);
    return material;
}
/** DanceFloor: lifetime pulse quads + tint × MainTex (preview GLSL). */
function buildDanceFloor({ mainTex, tintColor }) {
    const u = createTiltUniformBag({
        u_TintColor: { value: tintColor },
    });
    const material = new MeshBasicNodeMaterial();
...
function buildBubbleWand({ mainTex, scrollRate, scrollJitterIntensity, scrollJitterFrequency, }) {
    const u = createTiltUniformBag({
        u_ScrollRate: { value: scrollRate },
        u_ScrollJitterIntensity: { value: scrollJitterIntensity },
        u_ScrollJitterFrequency: { value: scrollJitterFrequency },
    ...
    const displaced = bubbleWandPositionLocal(u.u_time.node, u.u_ScrollRate.node, u.u_ScrollJitterIntensity.node, u.u_ScrollJitterFrequency.node, u.u_isNewTiltExporter.node);
    material.positionNode = displaced;
    const vColor = varying(brushColor(), 'v_tb_color');
    ...
    material.fragmentNode = Fn(() => {
        ...
        let rim = float(1).sub(abs(dot(viewDir, n)));
        rim = rim.mul(float(1).sub(pow(rim, float(5))));
        const diffraction = sampleMain(mainTex, vec2(rim.add(u.u_time.node.x).add(n.y), rim.add(n.y))).xyz;
        const emission = rim.mul(diffraction.mul(rim).mul(0.25).add(diffraction.mul(vColor.rgb).mul(0.75)));
...
function buildLacewing({ mainTex, bumpMap, specTex, cutoff, shininess, specColor, }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    ...
    material.fragmentNode = Fn(() => {
        ...
        const scroll = u.u_time.node.z;
        const animatedSpec = vec3(sin(specSample.x.mul(2).add(scroll.mul(0.5)).sub(vUv.x)).add(1), sin(specSample.x.mul(3.3).add(scroll).sub(vUv.x)).add(1), sin(specSample.x.mul(4.66).add(scroll.mul(0.25)).sub(vUv.x)).add(1));
        ...
        const lit = surfaceShaderSpecularGloss(bumped, normalize(ld0), normalize(vPos.negate()), u.u_SceneLight_0_color.node.rgb, albedo, specularColor, u.u_Shininess.node)
            .add(shShaderWithSpec(bumped, normalize(ld1), u.u_SceneLight_1_color.node.rgb, albedo, specularColor))
            .add(albedo.mul(u.u_ambient_light_color.node.rgb));
        const rgb = applyFog(lit, fogCoord, u.u_fogColor.node, u.u_fogDensity.node);
...
function buildMarbledRainbow({ mainTex, bumpMap, specTex, cutoff, shininess, specColor, }) {
    const u = createTiltUniformBag({
        u_Cutoff: { value: cutoff },
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
...
function buildMylarTube({ mainTex, shininess, specColor }) {
    const u = createTiltUniformBag({
        u_Shininess: { value: shininess },
        u_SpecColor: { value: specColor },
    });
    ...
    material.fragmentNode = Fn(() => {
        const n = normalize(vNormal);
        const viewDir = normalize(vPos.negate());
        const lit = surfaceShaderSpecularGloss(n, normalize(ld0), viewDir, u.u_SceneLight_0_color.node.rgb, vColor.rgb, u.u_SpecColor.node, u.u_Shininess.node)
            .add(shShaderWithSpec(n, normalize(ld1), u.u_SceneLight_1_color.node.rgb, vColor.rgb, u.u_SpecColor.node))
            .add(vColor.rgb.mul(u.u_ambient_light_color.node.rgb));
        const N = n.negate();
        const V = normalize(cameraPosition.sub(vPos));
        let rim = float(1).sub(abs(dot(V, N)));
        rim = rim.mul(float(1).sub(pow(rim, float(5))));
        const diffraction = sampleMain(mainTex, vec2(rim.add(u.u_time.node.y).add(N.y), rim.add(N.y))).rgb;
        const emission = rim.mul(diffraction.mul(rim).mul(0.25).add(diffraction.mul(vColor.rgb).mul(0.75)));
...
function buildRisingBubbles(params) {
 ...
 */
export function createNodeBrushMaterial(brushName, textures = {}, materialParams = {}) {
    ...
    switch (brushName) {
        ...
        case 'DoubleTaperedFlat':
            return buildDoubleTaperedFlat({
                ...
                specColor: uniforms.u_SpecColor?.value?.clone?.() ??
        ...
        case 'Comet':
            return buildComet({
                ...
                alphaMaskTexelSize: uniforms.u_AlphaMask_TexelSize?.value?.clone?.() ??
        ...
        case 'HyperGrid':
            return buildHyperGrid({
                mainTex,
                tintColor: uniforms.u_TintColor?.value?.clone?.() ??
        ...
        case 'Snow':
            return buildSnow({
                ...
                scrollDistance: uniforms.u_ScrollDistance?.value?.clone?.() ??
        ...
        case 'BlocksGlass':
            return buildBlocksGlass({
                ...
                color: uniforms.u_Color?.value?.clone?.() ??
        ...
        case 'PbrTemplate':
            return buildPbrTemplate({
                ...
                baseColorFactor: uniforms.u_BaseColorFactor?.value?.clone?.() ??
        ...
        case 'Plasma':
            return buildPlasma({
                mainTex,
                mainTexST: uniforms.u_MainTex_ST?.value?.clone?.() ??
        ...
        case 'FacetedTube':
            return buildFacetedTube({
                colorX: uniforms.u_ColorX?.value?.clone?.() ??
                    uniforms.u_ColorX?.value ??
                    new Vector4(1, 0, 0, 1),
                colorY: uniforms.u_ColorY?.value?.clone?.() ??
                    uniforms.u_ColorY?.value ??
                    new Vector4(0, 1, 0, 1),
                colorZ: uniforms.u_ColorZ?.value?.clone?.() ??
