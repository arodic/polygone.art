...
export class GLTFGoogleTiltBrushMaterialExtension {
     ...
     */
    applyLoadedBrushFlags(shader, { lights: _lights = true, fog: _fog = true } = {}) {
        if (!shader)
            return shader;
        shader.fog = false;
    ...
    async replaceMaterial(mesh, guidOrName, isNewTiltExporter = false) {
        const renameAttribute = (mesh, oldName, newName) => {
        ...
        const hasPbrUniforms = (prevUniforms?.u_BaseColorTex && prevUniforms?.u_MetallicFactor !== undefined) ||
        ...
        const isTransparentPbr = /PbrTransparentTemplate/i.test(mesh.material?.fragmentShader || '') ||
        ...
        const loadPbrTemplateMaterial = async () => {
            ...
            const baseColorTex = gltfUniforms?.u_BaseColorTex?.value?.isTexture
            ...
            for (const key of overrideKeys) {
                if (gltfUniforms?.[key]?.value === undefined)
                    continue;
            ...
            return createNodeBrushMaterial('PbrTemplate', { u_BaseColorTex: baseColorTex }, materialParams);
        };
        const loadPbrTransparentTemplateMaterial = async () => {
            ...
            return createNodeBrushMaterial('PbrTransparentTemplate', { u_BaseColorTex: baseColorTex }, materialParams);
        };
        const setAttributeIfExists = (mesh, oldName, newName, { warn = true } = {}) => {
        ...
        const ensureDiscoTexcoord1 = (mesh) => {
            if (mesh.geometry.getAttribute('a_texcoord1'))
                return;
            const pos = mesh.geometry.getAttribute('a_position')
                || mesh.geometry.getAttribute('position');
            if (!pos)
                return;
            mesh.geometry.setAttribute('a_texcoord1', new THREE.BufferAttribute(new Float32Array(pos.count * 3), 3));
        };
        ...
        const ensureTangentAttribute = (mesh) => {
            if (mesh.geometry.getAttribute('a_tangent'))
                return;
            const pos = mesh.geometry.getAttribute('a_position')
                || mesh.geometry.getAttribute('position');
            if (!pos)
        ...
        const ensureParticleCornerAttribute = (mesh) => {
            if (mesh.geometry.getAttribute('a_corner'))
                return;
            const pos = mesh.geometry.getAttribute('a_position')
                || mesh.geometry.getAttribute('position');
            if (!pos)
        ...
        const ensureQuadIndexAttribute = (mesh) => {
            if (mesh.geometry.getAttribute('a_quad'))
                return;
            const pos = mesh.geometry.getAttribute('a_position')
                || mesh.geometry.getAttribute('position');
            if (!pos)
            ...
            for (let i = 0; i < pos.count; i++)
                quads[i] = Math.floor(i / 4);
            mesh.geometry.setAttribute('a_quad', new THREE.BufferAttribute(quads, 1));
        };
        const ensureTimestampAttribute = (mesh) => {
            if (mesh.geometry.getAttribute('a_timestamp'))
                return;
            const pos = mesh.geometry.getAttribute('a_position')
                || mesh.geometry.getAttribute('position');
            if (!pos)
                return;
            mesh.geometry.setAttribute('a_timestamp', new THREE.BufferAttribute(new Float32Array(pos.count), 1));
        };
        const copyFixColorAttribute = (mesh) => {
            ...
            case "Disco":
                ...
                if (isNewTiltExporter) {
                    ...
                    if (!uv0HasRadius && !uv1HasBakedRadius) {
                        console.warn("Disco brush is missing radius data (need texcoord0.z or baked texcoord1.x). " +
                            "New exporter GLB likely lacks UV bake output.", mesh.name);
                    }
                }
        ...
        mesh.onBeforeRender = (renderer, scene, camera, _geometry, material, _group) => {
            ...
            const updateLight = (light, colorKey, matrixKey) => {
                if (!light)
                    return;
                ...
                setUniform(matrixKey, new THREE.Matrix4().lookAt(new THREE.Vector3(0, 0, 0), direction, new THREE.Vector3(0, 1, 0)));
            };
            ...
            if (mat?.uniforms?.directionalLights?.value) {
                ...
                if (mat.uniforms.directionalLights.value[0]) {
                    ...
                    if (mat.uniforms.u_SceneLight_0_matrix) {
                        ...
                        mat.uniforms.u_SceneLight_0_matrix.value = new THREE.Matrix4().lookAt(new THREE.Vector3(0, 0, 0), direction, new THREE.Vector3(0, 1, 0));
                    }
                }
                // Shadow Light
                if (mat.uniforms.directionalLights.value[1]) {
                    ...
                    if (mat.uniforms.u_SceneLight_1_matrix) {
                        ...
                        mat.uniforms.u_SceneLight_1_matrix.value = new THREE.Matrix4().lookAt(new THREE.Vector3(0, 0, 0), direction, new THREE.Vector3(0, 1, 0));
                    }
                }
            }
            // Ambient Light
            if (mat?.uniforms?.ambientLightColor?.value) {
                ...
                if (ambient) {
                    setUniform('u_ambient_light_color', new THREE.Vector4(ambient.color.r, ambient.color.g, ambient.color.b, 1));
                }
            }
            // Fog
            if (scene?.fog) {
                if (scene.fog.color)
                    setUniform('u_fogColor', scene.fog.color);
                const fogDensity = scene.fog.density;
                if (fogDensity != null)
                    setUniform('u_fogDensity', fogDensity);
            }
