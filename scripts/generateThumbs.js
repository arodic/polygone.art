...
async function generateThumbs() {
  ...
  for (let id in assets) {
    ...
    if (pngExists) await sharp(thumbnailPath)
      .resize(32)
      .jpeg({ mozjpeg: true, progressive: true, quality: 85 })
      .toBuffer((err, data, info) => {
        if (data) {
          const imgDataUri = data.toString('base64').replace('/9j/2wBDAAUFBQUFBQUGBgUICAcICAsKCQkKCxEMDQwNDBEaEBMQEBMQGhcbFhUWGxcpIBwcICkvJyUnLzkzMzlHREddXX3/2wBDAQUFBQUFBQUGBgUICAcICAsKCQkKCxEMDQwNDBEaEBMQEBMQGhcbFhUWGxcpIBwcICkvJyUnLzkzMzlHREddXX3/wgARCAAYACADASIAAhEBAxEB/', '');
